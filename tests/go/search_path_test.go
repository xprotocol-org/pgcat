...
func TestSearchPathIsolation(t *testing.T) {
	...
	var searchPathA string
	err = txA.QueryRow("SHOW search_path").Scan(&searchPathA)
	if err != nil {
		t.Fatalf("Get search_path A failed: %v", err)
	}
	t.Logf("Client A search_path: %s", searchPathA)
