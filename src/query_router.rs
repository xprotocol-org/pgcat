...
pub struct QueryRouter {
    ...
    // Advisory lock action and key extracted from Bind, ready to be set on server
    pending_advisory_lock_action: Option<(AdvisoryLockAction, Option<i64>)>,
}
...
impl QueryRouter {
    ...
    pub fn contains_session_advisory_lock(&mut self, message: &BytesMut) -> Option<(AdvisoryLockAction, Option<i64>)> {
        ...
        } else if query_lower.contains("pg_advisory_unlock") || query_lower.contains("pg_try_advisory_unlock") {
            AdvisoryLockAction::Unlock
        } else {
            AdvisoryLockAction::Lock
        };

        let (key, placeholders) = self.extract_advisory_lock_key_or_placeholders(&query);

        if !placeholders.is_empty() {
            debug!("Advisory lock query uses placeholders, will extract key from Bind message");
            self.pending_advisory_lock_parse = Some((action.clone(), placeholders));
            // Return None to indicate we need to wait for Bind message to get the key
    ...
    fn extract_advisory_lock_key_or_placeholders(&self, query: &str) -> (Option<i64>, Vec<i16>) {
        use regex::Regex;
    ...
    pub fn process_advisory_lock_bind(&mut self, message: &BytesMut) -> Option<(AdvisoryLockAction, Option<i64>)> {
        ...
        let key = if extracted_values.len() == 1 {
            Some(extracted_values[0])
        } else if extracted_values.len() == 2 {
            let k1 = extracted_values[0] as i32;
            let k2 = extracted_values[1] as i32;
            Some(((k1 as i64) << 32) | (k2 as i64 & 0xFFFFFFFF))
        } else {
            None
        };
    ...
    pub fn take_pending_advisory_lock_action(&mut self) -> Option<(AdvisoryLockAction, Option<i64>)> {
