        ...
        loop {
            ...
            match message[0] as char {
                ...
                'B' => {
                    if self.transaction_mode {
                        ...
                        if let Some((action, key)) = query_router.process_advisory_lock_bind(&message) {
                            debug!("Bind buffering: extracted advisory lock action: {:?}, key: {:?}", action, key);
            ...
            loop {
                ...
                match code {
                    ...
                    'Q' => {
                        if self.transaction_mode {
                            if let Some((action, key)) = query_router.contains_session_advisory_lock(&message) {
                                debug!("Query contains advisory lock operation (transaction mode), setting pending action");
                                server.set_pending_advisory_lock_action(action, key);
                            }
                        }
                        ...
                        if !server.in_transaction() && !server.has_advisory_lock() && !server.has_pinned_prepared_statements() {
                    ...
                    'B' => {
                        if self.transaction_mode {
                            if let Some((action, key)) = query_router.process_advisory_lock_bind(&message) {
                                debug!("Bind contains advisory lock parameters (transaction mode), action: {:?}, key: {:?}", action, key);
                                server.set_pending_advisory_lock_action(action, key);
                            } else {
                    ...
                    'S' => {
                        ...
                        if self.transaction_mode {
                            if let Some((action, key)) = query_router.take_pending_advisory_lock_action() {
                                debug!("Setting advisory lock action on server from buffered Bind: {:?}, key: {:?}", action, key);
                                server.set_pending_advisory_lock_action(action, key);
                            }
                        }
